...
import static com.dimas.util.Constant.*;
...
public class HeaderFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(@NonNull HttpServletRequest request) {
        return headerFilterProperties.getExcludedUrls().stream()
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws IOException {
        try {
            for (var headerName : GATEWAY_HEADER_NAMES) {
                ...
                if (isNull(header) && !headerFilterProperties.getOptionalGatewayHeaderNames().contains(headerName)) {
    ...
    private void setHeaderFromHeaderName(String header, String headerName) {
        ...
        switch (headerName) {
            case X_API_USER_ID_HEADER_NAME:
                headerStore.setUserId(Long.parseLong(header));
                break;
            case X_REQUEST_ID_HEADER_NAME:
                headerStore.setRequestId(header);
                break;
            default:
                break;
        }
